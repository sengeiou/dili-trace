...
//查询常用地址列表并填充到常用地址中
    ...
    function openUpdate(){
        ...
        $.ajax({
            type: "POST",
            url: " ${contextPath}/tradeDetail/findBase64Sign.action",
            data: {id:selected.id},
            processData:true,
            dataType: "json",
            async : true,
    ...
    function saveOrUpdate(){
        ...
        if(_formData.id == null || _formData.id==""){
            _url = "${contextPath}/tradeDetail/insert.action";
        }else{//有id就修改
            _url = "${contextPath}/tradeDetail/update.action";
            isNewData=true;
        }
    ...
    function del() {
        ...
        swal({
            ...
            $.ajax({
                type: "POST",
                url: "${contextPath}/tradeDetail/delete.action",
                data: {id: selected.id},
                processData:true,
                dataType: "json",
                async : true,
    ...
    function queryUserGrid() {
        ...
        if (null == opts.url || "" == opts.url) {
            opts.url = "${contextPath}/tradeDetail/listPage.action";
        }
    ...
    function initUserGrid() {
        ...
        var toolbar=[
            <#resource method="post" url="tradeDetail/index.html#add">
            {
                iconCls:'icon-add',
                text:'新增',
                handler:function(){
            ...
            <#resource method="post" url="tradeDetail/index.html#update">
            {
                iconCls:'icon-edit',
                text:'修改',
                handler:function(){
    ...
    function doDetail(){
        ...
        openWin('${contextPath}/tradeDetail/view/' + selected.id)
    }
