 ...
 function onBeforeLoad(param) {
    ...
    function findHighLightBill(){
    	highLightBill={};
		 $.ajax({
             type: "POST",
             url: "${contextPath}/registerBill/findHighLightBill.action",
             processData:true,
             dataType: "json",
             data:{},
             async : false,
    ...
    function initRegisterBillGrid() {
        ...
        <#resource method="post" url="registerBill/index.html#batchUndo">
            {
                iconCls:'icon-undo',
                text:'批量撤销',
                id:'batch-undo-btn',
                disabled :true,
                handler:registerBillGrid.doBatchUndo,
                handler:function(){
                    registerBillGrid.doBatchUndo();
                }
            },
        </#resource>
    ...
    function onClickRow(index,row) {
	    ...
	    var rows=$.makeArray(_registerBillGrid.datagrid("getSelections"));
        ...
        //batch
        if(registerBillGrid.rows.length>1){
            if(registerBillGrid.isBatchAudit()){
            	$('#batch-audit-btn').show();
            }
            if(registerBillGrid.isBatchSimpling()){
            	$('#batch-sampling-btn').show();
            }
            if(registerBillGrid.isBatchAuto()){
                $('#batch-auto-btn').show();
            }
            if(registerBillGrid.isBatchUndo()){
                $('#batch-undo-btn').show();
            }
            return;
        }
    ...
    function doUploadOrigincertifiy(){
        ...
        openWin('${contextPath}/registerBill/uploadOrigincertifiy/' + selected.id);
    }
    function doUploadDetectReport(){
        ...
        openWin('${contextPath}/registerBill/uploadDetectReport/' + selected.id);
    }
    function doCreateCheckSheet(){
        ...
        openWin('${contextPath}/checkSheet/edit.html?' +$.param({idList:idList},true));
    }
    function doCopy(){
        var selected = _registerBillGrid.datagrid("getSelected");
        if (null == selected) {
        ...
        openWin('${contextPath}/registerBill/copy.html?id=' + selected.id+"#copy");
    }
