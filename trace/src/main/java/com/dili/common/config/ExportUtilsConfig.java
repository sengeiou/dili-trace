...
@Configuration
public class ExportUtilsConfig {

	@Primary
	@Bean 
	ExportUtils createExportUtils() {
		ExportUtils eu=new ExportUtils() {

		    public final  Logger log = LoggerFactory.getLogger(ExportUtils.class);
...
//		                        .mediaType(MediaType.parse("application/x-www-form-urlencoded; charset=UTF-8"))
		            ...
		            if(resp.isSuccessful()){
			            ...
			            log.info("resp={}",new String(resp.body().bytes(),"UTF-8"));
			            log.info("resp={}",new String(resp.body().bytes(),"GBK"));
			            log.info("resp={}",new String(resp.body().bytes(),"ISO8859-1"));
